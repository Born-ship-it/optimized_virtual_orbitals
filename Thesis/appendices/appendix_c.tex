...
Here we give the full code implementation for OVOS in Pyhton, also see the GitHub repository at ... .


\begin{lstlisting}[style=python, caption={...}, label=lst:full_code]
"""
OVOS (Optimised Virtual Orbital Space) method

Minimises the second-order correlation energy (MP2) by rotating virtual orbitals.
...
class OVOS:
    """
    Minimises the MP2 correlation energy via orbital rotations (OVOS algorithm).
    ...
    num_opt_virtual_orbs : int
        Number of optimised virtual spin orbitals (active virtuals).
    ...
    hessian_reg : float, optional
        Regularisation added to Hessian diagonal when singular.
    """
    ...
    def _eri_vovo_antisym(self, mo_coeffs: List[np.ndarray]) -> np.ndarray:
        ...
        and antisymmetrise. Returns <ab||ij> in physicist notation (a,b,i,j).
        """
    ...
    def _compute_density(self, t_abij: np.ndarray) -> np.ndarray:
        """
        Virtual virtual density matrix D_ab = \sum_{i>j,c} t_ac^ij t_bc^ij.
        """
        nocc = t_abij.shape[2]
    ...
    def _hessian(self, t_abij: np.ndarray, eri_as: np.ndarray,
                 D_ab: np.ndarray, fock_spin: np.ndarray) -> np.ndarray:
        """
        Hessian H_{ae,bf} = 2\sum_{i>j} t_ab^ij <ef||ij>
                          - [\sum_{c,i>j} (t_ac^ij <bc||ij> + t_cb^ij <ca||ij>)] d_ef
                          + D_ab (f_aa + f_bb) d_ef
                          + D_ab f_ef (1 - d_ef)
        """
        vir = np.array(self.active_inocc_indices)
    ...
    def _rotate_orbitals(self, mo_coeffs: List[np.ndarray],
        ...
        assert np.allclose(U_spin.T @ U_spin, np.eye(n_spin), atol=1e-6), \
            "U^TU != I: Unitary transformation broken!"
        assert np.allclose(U_spin @ U_spin.T, np.eye(n_spin), atol=1e-6), \
            "UU^T != I: Unitary transformation broken!"
    ...
    def _canonicalize_active(self, mo_coeffs: List[np.ndarray],
        ...
        Does NOT touch inactive orbitals or reorder them, this preserves the active-inactive separation that the gradient/Hessian optimization assumes.
        ...
        while iter_count < self.max_iter:
            ...
            if iter_count == 1:
                ...
                if self.verbose and dgrad is not None:
                    ...
                    self._print(f"            dE = {self.dE:.2e}  ||grad|| = {grad_norm:.2e} {flag}")
            ...
            # Recanonicalize the active virtual block
