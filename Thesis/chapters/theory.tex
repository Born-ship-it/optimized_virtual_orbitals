...
This section provides the theoretical background necessary for understanding the Optimized Virtual Orbital Space (OVOS) method developed in this thesis. We will cover the mathematical framework of orbital rotations, the convergence criteria that defines optimal orbitals, and the optimisation strategies employed to find these orbitals. This foundation is critical for grasping how OVOS optimises the virtual orbital space to maximise correlation energy while maintaining computational efficiency. The concepts introduced here will be directly applied in the implementation of OVOS in Chapter~\ref{ch:method}.


\subsection{Orbital Rotations and Stationarity}

Orbital optimisation in post-HF methods relies on rotating the molecular orbitals to minimise a target energy functional, such as the MP2 correlation energy in the case of OVOS. Here we set up the rotation formalism and the stationarity condition needed to derive the OVOS equations.
...
As such we derived the explicit form of the second-order Hylleraas functional $J_2$ in terms of the MP2 amplitudes $t_{ij}^{ab}$, the Fock matrix elements $f_{pq}$, and the antisymmetrised two-electron integrals $\langle ij\|ab\rangle$. This functional serves as the target for orbital optimisation in OVOS, where the active virtual orbitals are rotated to minimise $J_2$.
...
Using the chain rule to evaluate the gradient, regarding the two-electron integrals and the Fock matrix elements, we get the intermediate expression:
\begin{align}
...
After inserting amplitude derivatives and simplifying (using symmetry and relabelling), this yields:
\begin{equation}
...
\end{itemize}
Overall, the per-iteration cost is dominated differently for a few cases, summarised in the following table:
\begin{table}[h]
    \centering
    \small
    \begin{tabular}{c|c|c|c|c}
...
\end{equation}
This condition implies that the MP2 energy is stationary with respect to orbital rotations that mix the active virtual orbitals (those included in the OVOS optimisation) and the inactive virtual orbitals (those excluded from the active space). In the context of orbital optimised MP2 (OO-MP2), stationarity of the energy with respect to all orbital rotations (including occupied-virtual) leads to the condition that the single-excitation amplitudes $T_1$ vanish, which defines Brueckner orbitals. 
