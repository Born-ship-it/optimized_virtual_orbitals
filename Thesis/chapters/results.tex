...
\begin{itemize}
    ...
    \item \textbf{Section 6.4} compares different initialisation strategies and their impact on final energies and convergence.
...
%   - Also check that $E_{\rm corr}^{\rm OVOS}(N_{\rm virt}^{\rm full}) = E_{\rm corr}^{\rm MP2}$
%     (full active space must reproduce full MP2 exactly).
%
% [TABLE – INCLUDE IN TEXT]
%   Columns: Molecule | Basis | $E_{\rm corr}^{\rm PySCF}$ |
%            $E_{\rm corr}^{\rm code}$ | $\delta E$ / $\mu E_h$
...
%    $N'_{\rm virt}/N_{\rm virt}$ for each test molecule and basis set."
%
% [WHAT TO COVER]
%   - Report $\eta$ curves for all 9 molecules at 6-31G and cc-pVDZ.
%   - Expected pattern: rapid initial rise, approaching 1 as $N' \to N_{\rm virt}$.
...
%
% [FIGURE – MAIN RESULT]
%   x-axis: $N'_{\rm virt}/N_{\rm virt}$ (fraction, 0 to 1)
...
%   - Report iteration counts vs.~$N'_{\rm virt}$: do larger spaces need more iterations?
%
% [FIGURE]
%   x-axis: Iteration number | y-axis: $|\Delta E_{\rm corr}|$ (log scale)
%   Caption: ``Convergence of the MP2 correlation energy for
%   H\textsubscript{2}O/cc-pVDZ with $N'_{\rm virt} = 8$ active virtual
%   spin-orbitals. The dashed line marks the convergence threshold $10^{-8}$~$E_h$.''
%
% [TABLE]
%   Columns: Molecule | Basis | $N'_{\rm virt}$ | Iterations | Converged?
...
\section{Effect of Initialisation Strategy}
% ---------------------------------------------------------------
% [SECTION INTRO – 6.4]
...
%
% [FIGURE]
%   x-axis: $N'_{\rm virt}$ | y-axis: $E_{\rm corr}$ (Hartree)
...
%
% [FIGURE]
%   x-axis: $N'_{\rm virt}/N_{\rm virt}$ | y-axis: $\eta$
...
%
% [TABLE]
%   Columns: $N'_{\rm virt}$ | $E_{\rm corr}^{\rm A\&B}$ |
...
%    and assesses empirical scaling with $N'_{\rm virt}$."
%
% [WHAT TO COVER]
...
%
% [FIGURE]
%   x-axis: $N_{\rm virt}$ (log scale) | y-axis: Wall-clock time / s (log scale)
%   Points for each molecule/basis; log-log linear fit.
%   Caption: ``Empirical scaling of OVOS wall-clock time with virtual
%   orbital count $N_{\rm virt}$. The line shows a power-law fit;
